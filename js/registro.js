...
async function registrar() {
    ...
    try {
        ...
        // 3. Guardar datos en tabla 'estudiantes'
        const { error: insertError } = await supabase
            .from('estudiantes')
            .insert([
                {
                    id: userId, // Mismo ID que en Auth
                    nombres_completos: nombres,
                    apellido_completo: apellidos,
                    cedula: cedula,
                    email: email,
                    fecha_nacimiento: fechaNacimiento,
                    usuario: email, // o el que quieras
                    clave: password, // Se encriptará automáticamente por el trigger
                    año: anio,
                    seccion: seccion,
                    lapso: lapso,
                    created_at: new Date()
